...
import { SparkTaskStatus, SparkTask } from '../types'
import { SparkService } from './index'

export class SparkScheduler {
  // 存储 taskId -> { timer, cancelled }
  private _tasks: Map<number, { timer: any; cancelled: boolean }> = new Map()
  ...
  private listenToCancelEvents() {
    this.ctx.on('spark/task-cancelled', (taskId: number) => {
  ...
  async start() {
    ...
    this.ctx.on('spark/task-created', (task: SparkTask) => {
  ...
  private scheduleTask(task: SparkTask) {
    const logger = this.ctx.logger('spark')
    // 确保 triggerTime 是 Date 对象
  ...
  private async executeTask(task: SparkTask) {
    const logger = this.ctx.logger('spark')
    ...
    try {
      ...
      if (useCharacter) {
        ...
        const success = await this.service.characterSandbox.execute(
          task.userId,
          task.channelId,
          guildId,
          task.content
   ...
   */
  private shouldUseCharacterSandbox(channelId: string): boolean {
    // 私聊不使用 character
   ...
   */
  private extractGuildId(channelId: string): string {
   ...
   */
  private async executeChatLunaSandbox(task: SparkTask) {
    const logger = this.ctx.logger('spark')
    ...
    await this.service.sandbox.execute(
      task.userId,
      task.channelId,
      task.content,
      room
  ...
  private async getRoom(userId: string, channelId: string) {
    try {
      let guildId: string
      let isGroup = false
