...
exports.CharacterSandbox = void 0;
...
class CharacterSandbox {
     ...
     * 执行影子会话
     */
    async execute(userId, channelId, guildId, prompt) {
        const logger = this.ctx.logger('spark:character-sandbox');
        try {
            const characterService = this.ctx.chatluna_character;
            if (!characterService) {
            ...
            for (const bot of this.ctx.bots) {
                try {
                    session = bot.session({
                        ...
                        elements: [koishi_1.h.text(triggerMessage)]
                    });
                    // 确保 session 有必要的属性
                    if (session) {
                        session.content = triggerMessage;
                        session.elements = [koishi_1.h.text(triggerMessage)];
                    }
                    selectedBot = bot;
            ...
            await this.ctx.parallel('chatluna_character/message_collect', session, messages);
...
exports.CharacterSandbox = CharacterSandbox;
