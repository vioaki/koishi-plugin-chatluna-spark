...
exports.isSessionInScope = isSessionInScope;
/**
 * 检查频道是否在作用域内
 ...
 * @returns 是否允许
 */
function isInScope(channelId, scope, isDirect, userId, guildId) {
...
function isSessionInScope(session, scope) {
    if (!scope)
        return true;
    return isInScope(session?.channelId, scope, session?.isDirect, session?.userId, session?.guildId);
}
