...
为 [ChatLuna](https://github.com/ChatLunaLab/chatluna) 添加提醒、跟进、定时任务、节日问候和主动聊天，并为 Character 补充节日问候。
...
- Character 节日桥接：将节日问候写入 Character 原生 wake-up 调度器

## 版本要求

- `koishi-plugin-chatluna >= 1.4.0-alpha.40`
- `koishi-plugin-chatluna-agent >= 1.0.41`
- Character 节日问候可选：`koishi-plugin-chatluna-character >= 0.0.230`
...
必须先启用 `database`、`chatluna` 和 `chatluna-agent`。需要 Character 节日问候时，再安装并启用 `chatluna-character`。
...
Character 群聊使用共享角色会话，不支持 `--personal`。Character target 只支持 `festival`；提醒、定时任务和主动聊天继续使用 Character 内置的 `wake_up_reply_*`、`next_reply` 和空闲触发。
...
Character target 的 features 只能设置为 `festival` 或 `none`。`spark.target.*` 需要管理员权限。
...
4. Spark 每小时检查下一节日，并只更新带 Spark 标记的 Character wake-up。
5. 删除或停用 Character target 时，Spark 只清理对应节日任务，不影响用户创建的 Character 任务。
6. `follow_up` 在同一会话的创建者先发消息时自动取消。
...
- Character 节日问候需要使用 `spark.target.add --character [名称]` 单独加入。
...
- 无法添加 Character target：确认 Character `0.0.230+` 已启用，且日志出现 `Spark Character festival bridge attached`。
